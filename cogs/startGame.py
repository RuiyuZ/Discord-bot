...
#         self.num_undercover = random.choice([1])
        self.num_undercover = random.choice([1, 1, 1, 2, 1, 1, 2, 5])
        self.teamA = Team('A', [], [])
        self.teamB = Team('B', [], [])
    ...
    async def game(self, ctx: discord.Interaction):
        if len(self.teamA.members) != 0 or len(self.teamB.members) != 0:
    ...
    async def vote(self, ctx):
        if self.msg_id is None or len(self.teamA.members) is 0 or len(self.teamB.members) is 0:
            await ctx.response.send_message("The game has not been started yet.")
            return
